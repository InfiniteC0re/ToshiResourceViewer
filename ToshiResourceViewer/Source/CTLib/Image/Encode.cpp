...
using PixelFunc = void (*)(const Buffer&, Buffer&, uint32_t, uint32_t, uint32_t, uint32_t);
using BlockFunc = void (*)(const Buffer&, Buffer&, uint32_t, uint32_t, uint32_t, uint32_t);
...
Buffer encodeI4(const Image& image)
{
    return encodeBase(image, 8, 8, -1,
        [](const Buffer& img, Buffer& data, uint32_t width, uint32_t height, uint32_t x, uint32_t y)
        {
            if (x >= width || y >= height)
            {
                if ((x & 1) == 0)
...
Buffer encodeRGBA8(const Image& image)
{
    return encodeBlock(image, 4, 4, 2,
        [](const Buffer& img, Buffer& block, uint32_t width, uint32_t height, uint32_t gx, uint32_t gy)
        {
...
void getDataBlock(Buffer& imgBlock, const Buffer& img, uint32_t w, uint32_t h, uint32_t x, uint32_t y)
{
