...
using PixelPaletteFunc = void ( * )( Buffer&, Buffer&, Buffer&, uint32_t, uint32_t, uint32_t, uint32_t );
	...
	for ( uint32_t gy = 0; gy < nh; gy += bh )
	{
		for ( uint32_t gx = 0; gx < nw; gx += bw )
		{
			for ( uint32_t y = 0; y < bh; ++y )
			{
				for ( uint32_t x = 0; x < bw; ++x )
				{
					func( data, palette, out, width, height, gx + x, gy + y );
				}
			}
		}
	}
...
{
	return decodeBase( data, width, height, 8, 8, []( Buffer& data, Buffer& out, uint32_t width, uint32_t height, uint32_t x, uint32_t y ) {
...
{
	return decodeBase( data, width, height, 8, 4, []( Buffer& data, Buffer& out, uint32_t width, uint32_t height, uint32_t x, uint32_t y ) {
		uint8_t grey = data.get();
		if ( x < width && y < height )
		{
			size_t off = ( ( y * width ) + x ) * 4;
...
{
	return decodeBase( data, width, height, 4, 4, []( Buffer& data, Buffer& out, uint32_t width, uint32_t height, uint32_t x, uint32_t y ) {
...
Image decodeCI4_IA8( Buffer& data, Buffer& palette, uint32_t width, uint32_t height )
{
	return decodeBasePalette( data, palette, width, height, 8, 8, []( Buffer& data, Buffer& palette, Buffer& out, uint32_t width, uint32_t height, uint32_t x, uint32_t y ) {
		...
		{
			coloridx = ( coloridx & 0xF );
		}

		palette.position( coloridx << 1 );
		uint8_t alpha = palette.get();
		uint8_t grey  = palette.get();
		if ( x < width && y < height )
		{
			size_t off = ( ( y * width ) + x ) * 4;
			out.put( off + 0, grey );
...
Image decodeCI4_RGB565( Buffer& data, Buffer& palette, uint32_t width, uint32_t height )
{
	return decodeBasePalette( data, palette, width, height, 8, 8, []( Buffer& data, Buffer& palette, Buffer& out, uint32_t width, uint32_t height, uint32_t x, uint32_t y ) {
		uint8_t coloridx = data.get();
		if ( ( x & 1 ) == 0 )
		...
		palette.position( coloridx << 1 );
		uint16_t rgb565 = palette.getShort();
		if ( x < width && y < height )
		{
			size_t off = ( ( y * width ) + x ) * 4;
			out.put( off + 0, ( rgb565 & 0xF800 ) >> 8 );
...
Image decodeCI4_RGB5A3( Buffer& data, Buffer& palette, uint32_t width, uint32_t height )
{
	return decodeBasePalette( data, palette, width, height, 8, 8, []( Buffer& data, Buffer& palette, Buffer& out, uint32_t width, uint32_t height, uint32_t x, uint32_t y ) {
		uint8_t coloridx = data.get();
		if ( ( x & 1 ) == 0 )
		...
		palette.position( coloridx << 1 );
		uint16_t rgb5a3 = palette.getShort();
		if ( x < width && y < height )
		{
			size_t off = ( ( y * width ) + x ) * 4;
			if ( ( rgb5a3 & 0x8000 ) == 0 ) // with alpha
...
Image decodeCI8_IA8( Buffer& data, Buffer& palette, uint32_t width, uint32_t height )
{
...
Image decodeCI8_RGB565( Buffer& data, Buffer& palette, uint32_t width, uint32_t height )
{
	return decodeBasePalette( data, palette, width, height, 8, 4, []( Buffer& data, Buffer& palette, Buffer& out, uint32_t width, uint32_t height, uint32_t x, uint32_t y ) {
		uint8_t coloridx = data.get();
...
Image decodeCI8_RGB5A3( Buffer& data, Buffer& palette, uint32_t width, uint32_t height )
{
	return decodeBasePalette( data, palette, width, height, 8, 4, []( Buffer& data, Buffer& palette, Buffer& out, uint32_t width, uint32_t height, uint32_t x, uint32_t y ) {
		uint8_t coloridx = data.get();
...
{
	return decodeBlock( data, width, height, 8, 8, -1, []( Buffer& block, Buffer& out, uint32_t width, uint32_t height, uint32_t gx, uint32_t gy ) {
		block.clear();
...
Image ImageCoder::decode( Buffer& data, Buffer& palette, uint32_t width, uint32_t height, ImageFormat format, ImageFormat paletteFormat )
{
	try
	{
		switch ( format )
		{
			...
			case ImageFormat::C4:
				if ( paletteFormat == ImageFormat::IA8 )
					return decodeCI4_IA8( data, palette, width, height );
				else if ( paletteFormat == ImageFormat::RGB5A3 )
					return decodeCI4_RGB5A3( data, palette, width, height );
				else if ( paletteFormat == ImageFormat::RGB565 )
					return decodeCI4_RGB565( data, palette, width, height );
				else
			...
			case ImageFormat::C8:
				if ( paletteFormat == ImageFormat::IA8 )
					return decodeCI8_IA8( data, palette, width, height );
				else if ( paletteFormat == ImageFormat::RGB5A3 )
